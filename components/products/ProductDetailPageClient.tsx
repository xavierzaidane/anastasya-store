...
export default function ProductDetailPageClient({ initialProduct }: Props) {
  ...
  const images = useMemo(() => {
    ...
    const all = [liveProduct.img, ...galleryImages.filter((image) => image !== liveProduct.img)].filter(Boolean);
    return all.length > 0 ? all : ["/placeholder.jpg"];
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <img
            ...
            alt={`${liveProduct.name} - image ${currentImageIndex + 1}`}
